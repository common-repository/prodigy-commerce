...
function prodigy_get_allowed_tags_for_logo_svg(): array {
	return array(
		'img'   => array(
 ...
 * @return string
 */
function prodigy_price_format( $price ): string {
 ...
 * @return string
 */
function prodigy_format_content( string $raw_string ): string {
 ...
 * @access public
 */
function prodigy_truncate( string $text, $length = 100, $options = array() ): string {
	$default = array(
		'ending' => '...',
		'exact'  => true,
		'html'   => true,
	...
	if ( $html ) {
		...
		$totalLength = mb_strlen( strip_tags( (string) $ending ) );
		$openTags    = array();
		$truncate    = '';
 ...
 * @return bool|string
 */
function prodigy_format_site_url( string $url ) {
 ...
 * @return string|array
 */
function prodigy_sanitize_field( $data = '', $type = 'text' ): string {
	if ( is_array( $data ) ) {
