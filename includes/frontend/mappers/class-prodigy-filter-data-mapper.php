...
class Prodigy_Filter_Data_Mapper extends Prodigy_Main_Data_Mapper {
	...
	const FILTER_BY_TITLE                        = 'Filter by';
	const ACTIVE_FILTER_TITLE                    = 'Active Filters';
	const PRICE_FILTER_TITLE                     = 'Price Filter';
	...
	public function http_build_by_attr( array $filters, string $attribute, string $attr_value ) {
		...
		if ( ( is_prodigy_product_taxonomy() || is_page( 'shop' ) ) && isset( $_SERVER['REQUEST_URI'] ) ) {
			...
			$path_redirect = Prodigy_Page::prodigy_get_shop_url();
		}
	...
	public function get_default_parameters( array $options = array() ): array {
		$query = '';
		$atts  = isset( $options['idWidget'] ) ? get_option( 'pg_elementor_filter_widget_' . $options['idWidget'] ) : $options;
		...
		$query_string = $products_parser->catalog_query_builder( $query_params, $products_parser->get_number_per_page() );
		$filters      = Prodigy_Request_Maker::get_instance()->do_catalog_filters_request( $query_string );
		...
		if ( isset( $atts['attrs_content_filter_button'] ) ) {
			$params['elementor_filter_mode'] = $atts['attrs_content_filter_button'] === 'button';
		} else {
			$params['elementor_filter_mode'] = false;
		}
