...
class Prodigy_Cart_Page_Data_Mapper extends Prodigy_Main_Data_Mapper {
	...
	public function format_data( array $cart_items ) :array {
		...
		foreach ( $cart_items as $key => $item_line ) {
			...
			$local_parent_product_info = Prodigy_Product_Parser::get_product_meta_by_remote_id(
			...
			$cart_items[ $key ]['data_analytics']['price']         = prodigy_price_format( $item_line['attributes']['price'] ?? 0 );
		}
