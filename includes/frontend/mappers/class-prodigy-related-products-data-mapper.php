...
class Prodigy_Related_Products_Data_Mapper extends Prodigy_Main_Data_Mapper {
	...
	public function get_default_parameters( array $options ): array {
		$options     = $this->set_type_params( $options );
		$options     = $this->set_order_params( $options );
		$options     = $this->set_pagination_params( $options );
		$product_ids = $this->get_type_of_products( $options );
		$heading     = apply_filters( 'prodigy_product_related_products_heading', 'Related products' );

		if ( ! empty( $options['style_image_ratio']['width'] ) && ! empty( $options['style_image_ratio']['height'] ) ) {
			$options['image_ratio'] = ( $options['style_image_ratio']['height'] / $options['style_image_ratio']['width'] ) * 100;
		}

		$options['products']        = $product_ids['data'] ?? array();
		$options['display']         = $options['display'] ?? self::SLIDER;
		...
		$options['heading']         = $options['style_content_heading_text'] ?? $heading ?? 'Related Products';
		$options['idWidget']        = $options['idWidget'] ?? '';
		$options['sale']            = $options['sale'] ?? '';
		$options['category']        = $options['category'] ?? true;
		$options['rating']          = $options['rating'] ?? true;
		$options['hide_arrows']     = $options['slider_hide_arrows'] ?? 'no';
		$options['placeholder_url'] = plugin_dir_url( PRODIGY_PLUGIN_PATH . 'web/admin/images/placeholder.png' ) . 'placeholder.png';
	 ...
	 * @return array
	 */
	public function set_type_params( array $params ): array {
	 ...
	 * @return array
	 */
	public function get_related_products( array $params ): array {
		if ( isset( $GLOBALS['prodigy_product'] ) ) {
			$products = array( $GLOBALS['prodigy_product']->get_remote_product_id() );
		} else {
			$products = $this->get_products_ids( Prodigy_Product_Template_Builder::get_random_products( self::UNLIMIT_PRODUCTS_NUMBER ) );
		}
		...
		return get_format_related_products( $up_sell_product_obj['data'] ?? array() );
	}
	 ...
	 * @return array
	 */
	public function get_products_ids( array $products ): array {
		$ids = array();
	...
	public function set_products_for_elementor_page( array $params ): array {
		$products             = Prodigy_Product_Template_Builder::get_random_products();
	 ...
	 * @return array
	 */
	public function set_order_params( array $params ): array {
	 ...
	 * @return array
	 */
	public function set_pagination_params( array $params ): array {
		...
		$params['pg'] = isset( $_GET['pg'] ) ? sanitize_key( wp_unslash( $_GET['pg'] ) ) : 1;
		...
		if ( isset( $params['display'] ) && $params['display'] === self::GRID ) {
			...
			$order_token          = isset( $_SERVER['HTTP_REFERER'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_REFERER'] ) ) : false;
			$base_url             = remove_query_arg( 'pg', $order_token );
	 ...
	 * @return string
	 */
	public function mappingOrderParams( string $order ): string {
		$hsParams = array(
			'id'         => 'id',
	 ...
	 * @return string
	 */
	public function mappingTypeParams( string $type ): string {
		$hsParams = array(
