...
class Prodigy_Cart_Data_Mapper extends Prodigy_Main_Data_Mapper {
	...
	public function get_default_parameters( array $options ): array {
		...
		return array(
			'heading_text'                 => $options['cart_style_heading_text'] ?? 'Shopping Cart',
			'empty_cart_text'              => $options['cart_style_empty_text'] ?? 'No products added yet',
			'cart_products'                => $cart_products ?? array(),
			'gallery_thumb'                => false,
			'is_dropdown'                  => $options['dropdown'] ?? true,
			...
			'count_classname'              => $options['count_classname'] ?? 'prodigy-navbar-cart__count',
			'attr_shortcode'               => $options,
			'cart_icon_type'               => $options['cart_icon_type'] ?? '',
			'cart_content_icon_hide_empty' => $options['cart_content_icon_hide_empty'] ?? '',
