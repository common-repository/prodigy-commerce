	...
	public function format_data( array $products ) :array {
		...
		if ( ! empty( $products ) ) {
			foreach ( $products as $key => $product ) {
				$local_product                                  = Prodigy_Product_Parser::get_product_meta_by_remote_id( 'prodigy_remote_product_id', (int) $product['id'] );
				...
				$products_data[ $key ]['versions-image-url']    = $product['attributes']['versions-image-url'] ?? [];
				$products_data[ $key ]['rating']                = $product['attributes']['rating'];
				$products_data[ $key ]['tiered-price']          = $product['attributes']['tiered-price'];
				$products_data[ $key ]['price-range']           = $product['attributes']['price-range'] ?? [];
				$products_data[ $key ]['logo-location']         = $product['attributes']['logo-location'] ?? [];
				$products_data[ $key ]['logo']                  = $product['attributes']['logo'] ?? [];
				$products_data[ $key ]['image-cropping-params'] = $product['attributes']['image-cropping-params'] ?? [];
